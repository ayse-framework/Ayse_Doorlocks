...
function nearbyDoorUnlock()
    ...
    doorInfo.state = 0
    TriggerServerEvent("Ayse_Doorlocks:setState", doorNear, doorInfo.state)
end
...
RegisterNetEvent("Ayse:setCharacter", function(character)
    doorsResetDefault()
    job = character.job
    TriggerServerEvent("Ayse_Doorlocks:getDoors")
end)

RegisterNetEvent("Ayse_Doorlocks:syncDoor", function(id, state)
...
RegisterNetEvent("Ayse_Doorlocks:returnDoors", function(newSyncedDoors)
        ...
        for id, doorInfo in pairs(updatedDoors) do
            if #(pedCoords - doorInfo.textCoords) < doorInfo.accessDistance then
                ...
                if hasAccess and IsControlJustPressed(0, 51) then
                    ...
                    TriggerServerEvent("Ayse_Doorlocks:setState", id, doorInfo.state)
                end
            end
        end
        Wait(0)
